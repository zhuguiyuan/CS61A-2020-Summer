...
class TACard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        300
        """
        others_attack = other_card.attack
        other_card.attack = other_card.defense
        other_card.defense = others_attack
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        player_deck_length = len(player.deck.cards)
        attack = other_card.attack
        defense = other_card.defense
        for i in range(player_deck_length):
            player.deck.cards[i].attack += attack
            player.deck.cards[i].defense += defense
        for card in opponent.deck.cards[:]:
            if attack == card.attack or defense == card.defense:
