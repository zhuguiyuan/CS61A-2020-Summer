...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    correct_words_nums, reference_words_nums = 0, len(typed_words)
    if reference_words_nums == 0:
        return 0.0
    for typed_word, reference_word in zip(typed_words, reference_words):
        if typed_word == reference_word:
            correct_words_nums += 1
    return correct_words_nums/reference_words_nums * 100
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    return_word, lowest_diff = user_word, float('inf')
    for word in valid_words:
        diff = diff_function(user_word, word, limit)
        if diff < lowest_diff:
            return_word, lowest_diff = word, diff
    return return_word if lowest_diff <= limit else user_word
    # END PROBLEM 5
...
def report_progress(typed, prompt, id, send):
    ...
    # BEGIN PROBLEM 8
    cnt, min_size = 0, min(len(typed), len(prompt))
    while cnt < min_size and typed[cnt] == prompt[cnt]:
        cnt += 1
...
def time_per_word(times_per_player, words):
    ...
    for times in times_per_player:
        all_times.append([times[i]-times[i-1] for i in range(1, len(times))])
...
def fastest_words(game):
    ...
    for _ in players:
        fastest.append([])
    for word in words:
        min_time, fastest_player = time(game, 0, word), 0
        for player in players:
            if time(game, player, word) < min_time:
                min_time, fastest_player = time(game, player, word), player 
