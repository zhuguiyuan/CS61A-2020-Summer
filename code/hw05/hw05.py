...
def make_withdraw(balance, password):
    ...
    def protected_withdraw(amount, password):
        if password in incorrect_passwords:
            return 'Incorrect password'
        elif len(incorrect_passwords) >= 3:
            return f'Too many incorrect attempts. Attempts: {incorrect_passwords}'
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    "Too many incorrect attempts. Attempts: ['my', 'secret', 'password']"
    """
    def joint_withdraw(amout, password):
        if password == new_pass:
            return withdraw(amout, old_pass)
        return withdraw(amout, password)
    varify = withdraw(0, old_pass)
    if type(varify) == str:
        return varify
