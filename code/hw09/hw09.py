...
def in_order_traversal(t):
    ...
    return in_order_traversal(left) + [t.label] + in_order_traversal(right)
...
def interleaved_sum(n, odd_term, even_term):
    ...
    True
    """
    def helper(sum, i, fn1, fn2):
        if i > n:
            return sum
        return helper(sum + fn1(i), i + 1, fn2, fn1)
    return helper(0, 1, odd_term, even_term)
...
def mutate_reverse(link):
    ...
    """
    def helper(todo, done):
        if todo is Link.empty:
            return done
        return helper(todo.rest, Link(todo.first, done))

    link.rest = helper(link.rest, Link.empty)
