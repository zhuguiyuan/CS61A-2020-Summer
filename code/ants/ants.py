...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        # BEGIN Problem 9
            ...
            ant_to_contain = place.ant
            place.ant = self
            self.contain_ant(ant_to_contain)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        distance = 0
        while place is not None and place is not beehive:
            if self.min_range <= distance <= self.max_range \
            and len(place.bees) != 0:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        super().__init__(armor)
        if QueenAnt.the_only_queen is None:
            QueenAnt.the_only_queen = self
        # END Problem 13
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 13
        if self is not self.the_only_queen:
    ...
    def buff(self, ant):
        """Buff an ant and it's contained ant (might be None)."""
        if ant is not None and ant not in self.buffed_ants:
            ant.damage = ant.damage * 2
            self.buffed_ants.append(ant)
        if isinstance(ant, ContainerAnt):
            self.buff(ant.contained_ant)
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        super().reduce_armor(amount)
        if self.the_only_queen is self and self.armor <= 0:
            bees_win()
        # END Problem 13
    
    def remove_from(self, place):
        """True QueenAnt cannot be removed."""
        if self is not self.the_only_queen:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        damage = self.damage - self.insects_shot * 0.05 - distance * 0.2
        return max(damage, 0)
        # END Problem OPTIONAL
