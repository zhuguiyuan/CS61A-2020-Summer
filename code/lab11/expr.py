...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        if operator_val is None:
            raise NameError(f"Oops, name '{self.operator}' is None.")
        operands_val = [operand.eval(env) for operand in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        this_frame =self.parent.copy()
        for name, value in zip(self.parameters, arguments):
            this_frame[name] = value
        return self.body.eval(this_frame)
