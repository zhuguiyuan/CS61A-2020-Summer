def prune_min(t):
    ...
    """
    t.branches = sorted(t.branches, key=lambda b: b.label)[:1]
    if len(t.branches) > 0:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        result, cost = f'+[{code_char}]' + result, cost + 1
        possibilities.append((result, cost))
        # Delete
        result, cost = helper_align(skeleton_idx + 1, code_idx)
        result, cost = f'-[{skel_char}]' + result, cost + 1
        possibilities.append((result, cost))
...
def num_splits(s, d):
    ...
    def helper(lst):
        if len(lst) == 0:
            return [[]]
        without_first = [comb for comb in helper(lst[1:])]
        with_first = [[lst[0]] + c for c in without_first]
        return without_first + with_first
    num, total_sum = 0, sum(s)
    for lst in helper(s):
        sum_a, sum_b = sum(lst), total_sum - sum(lst)
        num += 1 if sum_b - d <= sum_a <= sum_b + d else 0
    return num // 2
