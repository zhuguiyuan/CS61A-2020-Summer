...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        # BEGIN PROBLEM 10
        validate_form(vals, len(formals), len(formals))
        child = Frame(self)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong arguments was passed: {0}'.format(args))
        # END PROBLEM 3
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    value = True
    while expressions is not nil and is_true_primitive(value):
        # tail recursion optimization
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 12
    value = False
    while expressions is not nil and is_false_primitive(value):
        # tail recursion optimization
        if expressions.rest is nil:
            value = scheme_eval(expressions.first, env, True)
        else:
